...
本文件定义了开发日志的标准格式，包括正常完成、失败/未完成以及接管前人中断任务等场景。
日志**统一写入 `docs/handoff-log.md`**（逆序，最新在上；超 10 条由归档工具或 Agent 移入 `docs/handoff-log-archive.md`）。
...
- **执行 Agent**：[例如 Claude Code, Codex, Cursor, Antigravity 等]
- **会话目标**：本次会话计划完成的任务。
- **改动文件清单**：修改/新增/删除的**所有**文件（路径级，方便回看与恢复）。
- **具体改动**：实现的功能或修复的问题。
- **环境与依赖变更**：[无 / 新增了 xxx 依赖包 / 更新了 .env.example / 修改了环境变量]
- **技术决策**：关键实现思路、折中考量、新增配置。
- **破坏性变更/注意事项**：[无 / 某公开函数参数调整，调用方需注意...]
...
当发现上一个 Agent 的会话异常中断，遗留了处于进行中（`[/]`）的半成品任务时：

```markdown
### YYYY-MM-DD HH:mm (接管中断任务摘要)
- **执行 Agent**：[Agent 名称]
- **接管目标**：接管上轮会话遗留的任务 [任务简述]。
- **现场评估**：
  - 代码实际状态：[通过 git status/测试确认前序 Agent 做到哪一步]
  - 任务看板调整：[将上轮遗留的 [/] 恢复为当前接手或转为 [!] 阻塞]
- **本次补齐改动**：[本次会话完成的工作与补齐的代码]
- **改动文件清单**：[本次修改的文件]
- **项目现状**：构建状态、测试状态。
- **接班任务**：后续待办。
```
...
| 交接日志更新 | `docs: handoff log YYYY-MM-DD` | `docs: handoff log 2026-09-08` |
...
4. 日志文件只保留最近 10 条。
5. 此操作可用 `node scripts/validate-handoff.mjs --archive` 自动完成，无 Node 环境时由 Agent 手动执行。
